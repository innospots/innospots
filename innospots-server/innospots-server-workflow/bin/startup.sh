...
#Spring profile
export PROFILE="dev"
export SERVER_MAIN_CLASS="io.innospots.workflow.runtime.server.InnospotsWorkflowServer"
...
export LOG_FILE="${LOG_DIR}/innospot.log"
...
CLASSPATH="${CLASSPATH}:${APP_DIR}/lib/*:${APP_DIR}/config:${INNOS_ROOT}/ext_config:${INNOS_ROOT}/ext_lig/*"

if [[ "${PROFILE}" == "dev" ]]; then
  JAVA_OPT="-Xms128m -Xmx1g -Xss512k"
else
  JAVA_OPT="-Xms2g -Xmx2g -Xss512k -Xmn1g -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=320m"
...
initLog() {
  ...
  echo "log out directory: ${LOG_DIR}"
...
startup() {
  ...
  echo "================================================================================================================"
  if [[ ${TPID} -ne 0 ]]; then
    echo "${SERVER_MAIN_CLASS} already started (PID=${TPID})"
    echo "================================================================================================================"
  else
    rmPid
    ...
    getPid
    if [[ ${TPID} -ne 0 ]]; then
      echo "${SERVER_MAIN_CLASS} (PID=${TPID})...[Success]"
      echo "================================================================================================================"
      echo "tail log file : ${LOG_FILE}"
    else
      echo -e "\033[31m ${SERVER_MAIN_CLASS} boot failed. [Failed] \033[0m"
      echo "================================================================================================================"
      rmPid
    fi
  fi
